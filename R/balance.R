balance<-function(fdata,mdata,fz,mz){
  ...
  smdf<-(meanf1-meanf0)/sqrt((sdf0^2+sdf1^2)/2)
  smdm<-(meanm1-meanm0)/sqrt((sdf0^2+sdf1^2)/2)

  r<-cbind(meanf1,meanf0,meanm1,meanm0,smdm,smdf)
  rownames(r)<-colnames(fdata)
  colnames(r)<-c('Treated Mean','Control Mean','Treated Match Mean','Control Match Mean',
                 'Match SMD','All SMD')
  r
}
